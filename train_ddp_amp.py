...
def validate(model, criterion, valid_loader):
    ...
    with torch.no_grad():
        ...
        for i, batch in enumerate(valid_loader):
            ...
            val_sum += loss.item()
            val_mel += loss_meta[0].item()
            val_gate += loss_meta[1].item()
        val_sum = val_sum / (i + 1)
        val_mel = val_mel / (i + 1)
        val_gate = val_gate / (i + 1)

    model.train()
    return (val_sum, val_mel, val_gate)
...
def train(
    ...
    for epoch in range(epoch_offset, hparams.epochs):
        ...
        for batch in train_loader:
            ...
            accum_loss += loss
            accum_loss_mel += loss_meta[0]
            accum_loss_gate += loss_meta[1]
            train_loss = (accum_loss, accum_loss_mel, accum_loss_gate)
                ...
                if gpu_rank == 0:
                    print(
                        "Train loss {} {:.6f} Grad Norm {:.6f} {:.2f}s/it".format(
                            accum_iter, accum_loss.item(), grad_norm, accum_dur
                        )
                    )
                    logger.log_training(
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        "--output_directory",
        type=str,
        default="/home/server/disk1/checkpoints/tacotron2/exp_ddp",
    ...
    parser.add_argument(
        ...
        "--log_directory",
        type=str,
        default="/home/server/disk1/checkpoints/tacotron2/log/exp_ddp",
